...
namespace LogicEntity.Default.MySql
{
    public partial class LinqConvertProvider
    {
        readonly Dictionary<MemberInfo, object> MemberFormat = new()
        {
            ...
            { typeof(string).GetMethod(nameof(string.Contains), new Type[]{ typeof(char) }), "({0} Like {1})" },
            { typeof(string).GetMethod(nameof(string.Contains), new Type[]{ typeof(string) }), "({0} Like {1})" },
